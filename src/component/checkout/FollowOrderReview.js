...
export default function FollowOrderReview({ back = null }) {
    const { t } = useTranslation();
    const orderContext = useContext(OrderContext);
    const { cart, total_order, current_order } = orderContext;

    const navigate = useNavigate()

    const steps = [t('followOrder.preparationOnGoing'), t('followOrder.sent'), t('followOrder.deliverOnGoing'), t('followOrder.delivered')];
            ...
            <List disablePadding>
                {cart.map((item, id) => (
                    <ListItem key={item.product.title} sx={{ py: 1, px: 0 }}>
                        <img src={require(`../../images/${item.product.image}`)} key={id} style={{ maxWidth: 150 }} alt="article image"/>
